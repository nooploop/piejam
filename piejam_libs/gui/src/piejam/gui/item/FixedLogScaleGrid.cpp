...
struct FixedLogScaleGrid::Impl
{
    bool scaleIsDirty{};
    bool colorIsDirty{};
};

FixedLogScaleGrid::FixedLogScaleGrid(QQuickItem* parent)
    : QQuickItem(parent)
    ...
    , m_impl{make_pimpl<Impl>()}
{
    ...
    connect(this, &FixedLogScaleGrid::widthChanged, this, [this]() {
        m_impl->scaleIsDirty = true;
        update();
    });

    connect(this, &FixedLogScaleGrid::heightChanged, this, [this]() {
        m_impl->scaleIsDirty = true;
        update();
    });

    connect(this, &FixedLogScaleGrid::colorChanged, this, [this]() {
        m_impl->colorIsDirty = true;
        update();
    });

    connect(this, &FixedLogScaleGrid::ticksChanged, this, [this]() {
...
auto
FixedLogScaleGrid::updatePaintNode(QSGNode* oldNode, UpdatePaintNodeData*)
        -> QSGNode*
{
    ...
    if (m_impl->scaleIsDirty)
    {
        ...
        auto const min = std::log10(m_ticks.front());
        auto const max = std::log10(m_ticks.back());
        ...
        if (m_orientation == Qt::Orientation::Horizontal)
        {
            ...
            {
                auto const x = ((std::log10(tick) - min) / diff) * w;
                vertices[0].x = x;
                vertices[0].y = 0;
                vertices[1].x = x;
                vertices[1].y = h;
        ...
        else
        {
            for (auto tick : qAsConst(m_ticks))
            {
                auto const y = ((std::log10(tick) - min) / diff) * h;
                vertices[0].x = 0;
                vertices[0].y = y;
                vertices[1].x = w;
                vertices[1].y = y;
